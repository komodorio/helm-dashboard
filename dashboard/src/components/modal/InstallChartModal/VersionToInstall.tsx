...
export const VersionToInstall: React.FC<{
    versions: NonEmptyArray<{ repository: string; version: string; isChartVersion: boolean }>;
    onSelectVersion: (version: string) => void;
  }> = ({ versions, onSelectVersion }) => {
    const chartVersion = versions.find(({ isChartVersion }) => isChartVersion)!.version;
    ...
    return (<div className="flex gap-2 text-xl items-center">
        {versions?.length ? (
        <>
          Version to install:{' '}
          <Select
            ...
            components={{
              SingleValue: ({ children, ...props }) => (
                <components.SingleValue {...props}>
                  ...
                  {props.data.check && <BsCheck2 className="inline-block ml-2 text-green-700 font-bold" />}
              ...
              Option: ({ children, isSelected, innerProps, data }) => (
                ...
                >
                  ...
                  {data.check && <BsCheck2 fontWeight={'bold'} className="inline-block ml-2 text-green-700 font-bold" />}
                </div>
              )

...
export const InstallChartModal = ({
    ...
    const { error: versionsError, data: _versions } = useGetVersions(
        chartName,
        {
            select: (data) => {
        ...
        async () => {
            ...
            const res = await fetch(
                ...
                `/api/helm/releases/${namespace ? namespace : 'default'}${
                    !isInstall
                        ? `/${releaseName}`
                        : `/${releaseValues ? chartName : ''}` // if there is no release we don't provide anything, and we dont display version
                }`,
                {
                    method: 'post',
                    body: formData,
